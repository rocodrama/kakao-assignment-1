...
export default function TodoEditForm({
  ...
  const [date, setDate] = useState(initialTodo?.date ?? "");
  const [isCompleted, setIsCompleted] = useState(initialTodo?.is_completed ?? false);

  if (!initialTodo) {
    return (
      <div className="flex flex-col items-center gap-4">
        <p className="text-sm text-primary/60">수정할 할 일을 찾을 수 없어요</p>
        <Link href="/todos" className="text-sm font-medium text-primary underline">
          목록으로 돌아가기
        </Link>
      </div>
    );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await fetch(`${API_URL}/todos/${todoId}`, {
      ...
      body: JSON.stringify({ title: trimmed, date, is_completed: isCompleted }),
    });
    router.push("/todos");
  }
